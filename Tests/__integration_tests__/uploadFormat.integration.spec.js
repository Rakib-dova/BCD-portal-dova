...
describe('ルーティングのインテグレーションテスト', () => {
  ...
  describe('4.契約ステータス：契約中', () => {
    ...
    test('基本情報設定画面入力：伝送されたCSVファイルチェック、ヘッダあり', async () => {
    ...
    test('基本情報設定画面入力：必須項目チェック', async () => {
      ...
      const errormessageTag = await page.evaluate(() => {
        const requiredItem = document.querySelectorAll('.requiredItem')
        const result = []
        requiredItem.forEach((item) => {
          result.push(item.parentNode.parentNode.children[0].children[2].innerText)
        })
        return result
      })
