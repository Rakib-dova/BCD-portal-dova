...
describe('部門データ一括作成のインテグレーションテスト', () => {
  let acCookies
  let userCookies
  let testTenantId
  ...
  describe('1.契約ステータス：未登録', () => {
    // 利用登録をしていないため、部門データ一括作成ページ利用できない
    test('管理者、契約ステータス：未登録、GET利用不可', async () => {
      const res = await request(app)
        .get('/uploadDepartment')
  ...
  describe('2.契約ステータス：登録申込', () => {
    // テナントステータスが「新規申込」、部門データ一括作成ページ利用できる
  ...
  describe('3.契約ステータス：登録受付', () => {
    // テナントステータスが「登録受付」、部門データ一括作成ページ利用できる
  ...
  describe('4.契約ステータス：契約中', () => {
    // テナントステータスが「契約中」、部門データ一括作成ページ利用できる
    test('管理者、契約ステータス：契約中、GET利用可能', async () => {
      ...
      const testDepartmentCode = new db.DepartmentCode({
        departmentCodeId: v4(),
        contractId: contract.contractId,
        departmentCodeName: 'インテグレーションテスト',
        departmentCode: 'INTE0001'
      })
      await testDepartmentCode.save()
    ...
    test('部門データ一括作成：バリデーションチェック機能（部門コード、部門名）', async () => {
      const puppeteer = require('puppeteer')
      const browser = await puppeteer.launch({
        headless: true,
        ignoreHTTPSErrors: true
      ...
      await fileChooser.accept(['./testData/departmentCodeUpload_inegration1.csv'])
    ...
    test('部門データ一括作成：ヘッダチェック', async () => {
      ...
      await fileChooser.accept(['./testData/departmentCodeUpload_inegration2.csv'])
    ...
    test('部門データ一括作成：項目数チェック', async () => {
      ...
      await fileChooser.accept(['./testData/departmentCodeUpload_inegration3.csv'])
    ...
    test('部門データ一括作成：200件件数超過', async () => {
      ...
      await fileChooser.accept(['./testData/departmentCodeUpload_test4.csv'])
      ...
      // エラーメッセージ確認
      expect(errorMsg).toMatch('部門データが200件を超えています。')
      expect(errorMsg).toMatch('CSVファイルを確認後もう一度アップロードしてください。')
      expect(errorMsg).toMatch('（一度に取り込める部門データは200件までとなります。）')
  ...
  describe('5.契約ステータス：変更申込', () => {
    // テナントステータスが「変更申込」、部門データ一括作成ページ利用できる
  ...
  describe('5.契約ステータス：変更受付', () => {
    // テナントステータスが「変更受付」、部門データ一括作成ページ利用できる
  ...
  describe('5.契約ステータス：解約申込', () => {
    // テナントステータスが「解約申込」、部門データ一括作成ページ利用できない
  ...
  describe('5.契約ステータス：解約受付', () => {
    // テナントステータスが「解約受付」、部門データ一括作成ページ利用できない
  ...
  describe('5.契約ステータス：解約', () => {
    // テナントステータスが「解約」、部門データ一括作成ページ利用できない
