...
describe('勘定科目作成のインテグレーションテスト', () => {
  ...
  describe('1.契約ステータス：未登録', () => {
    // 利用登録をしていないため、勘定科目作成ページ利用できない
    test('管理者、契約ステータス：未登録、利用不可', async () => {
      const res = await request(app)
        .get('/registAccountCode')
  ...
  describe('3.契約ステータス：登録申込', () => {
    ...
    test('管理者、契約ステータス：登録申込、利用可能', async () => {
      ...
      // 画面内容確認
      expect(res.text).toMatch(/勘定科目/i)
