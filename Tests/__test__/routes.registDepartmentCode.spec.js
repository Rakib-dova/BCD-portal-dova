...
const registDepartmentCode = require('../../Application/routes/registDepartmentCode')
...
let userControllerFindOneSpy, contractControllerFindOneSpy, departmentCodeControllerInsertSpy, checkContractStatusSpy
...
describe('registAccountCodeのテスト', () => {
  ...
  describe('コールバック:cbGetRegistDepartmentCode', () => {
    test('正常', async () => {
      // 準備
  ...
  describe('コールバック:cbPostRegistDepartmentCode', () => {
    test('正常', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
      request.body = {
        setDepartmentCodeInputId: 'DEP001',
        setDepartmentCodeNameInputId: 'setDepartmentCodeNameInputId'
      }
      ...
      // 試験実施
      await registDepartmentCode.cbPostRegistDepartmentCode(request, response, next)
    ...
    test('異常:部門データ登録に失敗した場合。', async () => {
      ...
      expect(request.session?.userRole).toBe('a6a3edcd-00d9-427c-bf03-4ef0112ba16d')
      // responseのヘッダ
      expect(request.flash).toHaveBeenCalledWith('noti', ['部門データ登録', '部門データ登録に失敗しました。'])
      expect(response.getHeader('Location')).toEqual('/registDepartmentCode')
    })
    ...
    test('500エラー:accountCodeController.insertエラー', async () => {
      ...
      departmentCodeControllerInsertSpy.mockReturnValue(dbError)
      // ユーザ権限チェック結果設定
      checkContractStatusSpy.mockReturnValue(Contracts[0].dataValues.contractStatus)
      // 試験実施
      await registDepartmentCode.cbPostRegistDepartmentCode(request, response, next)
