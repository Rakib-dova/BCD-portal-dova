...
const deleteApproveRoute = require('../../Application/routes/deleteApproveRoute')
...
describe('deleteApproveRouteのテスト', () => {
  ...
  describe('cbDeleteApproveRoute', () => {
    test('正常：削除完了', async () => {
      ...
      // 削除結果（Mock）
      deleteApproveRouteSpy.mockReturnValue(1)

      // 承認ルート削除実施
      await deleteApproveRoute.cbDeleteApproveRoute(request, response, next)

      // request.flashが呼ばれ「る」
      expect(request.flash).toHaveBeenCalledWith('info', '承認ルートを削除しました。')
    ...
    test('準正常:DBエラー発生', async () => {
      ...
      // 削除結果（Mock）
      deleteApproveRouteSpy.mockReturnValue(0)
