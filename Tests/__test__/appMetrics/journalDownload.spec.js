...
describe('アプリ効果測定UT_デジトレ', () => {
  ...
  describe('仕分情報ダウンロード', () => {
    test('請求書番号を指定しない場合', async () => {
      ...
      journalDownloadSpy.mockResolvedValue([{
        invoice: {
          InvoiceLine: [{
            ID: { value: null },
            InvoicedQuantity: { value: null }
          }],
          IssueDate: { value: null },
          AccountingCustomerParty: { Party: { PartyIdentification: null } },
          AccountingSupplierParty: { Party: { PartyIdentification: null } },
          ID: { value: null }
        },
        journalizeInvoiceFinal: []
      }])
      accessTradeshiftSpy
