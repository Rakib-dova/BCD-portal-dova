...
describe('csvDownloadのテスト', () => {
  ...
  describe('コールバック:cbPostIndex', () => {
    ...
    test('正常:受信企業あり（複数）・送信企業あり（複数）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
      request.body = {
        ...
        minIssuedate: '2021-08-01',
        maxIssuedate: '2021-11-09',
        sentTo: ['f783be0e-e716-4eab-a7ec-5ce36b3c7b31', 'f783be0e-e716-4eab-a7ec-5ce36b3c7b31'], // 同一テナントIDで複数を実現
        sentBy: ['221559d0-53aa-44a2-ab29-0c4a6cb02bde', '221559d0-53aa-44a2-ab29-0c4a6cb02bde'] // 同一テナントIDで複数を実現
      }
    ...
    test('正常:条件検索（ステータス - 入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・受理済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・送金済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 受理済み・送金済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 受理済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送金済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・受理済み・送金済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・受理済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
      request.body = {
        invoiceNumber: 'A01001',
        status: [status[0], status[1], status[3]],
        buyAndSell: buyAndSell[0],
        minIssuedate: '2021-08-01',
        maxIssuedate: '2021-11-09'
      }
    ...
    test('正常:条件検索（ステータス - 送信済み/受信済み・送信済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
      request.body = {
        invoiceNumber: 'A01001',
        status: [status[0], status[2], status[3]],
        buyAndSell: buyAndSell[0],
        minIssuedate: '2021-08-01',
        maxIssuedate: '2021-11-09'
      }
    ...
    test('正常:条件検索（ステータス - 受理済み・送信済み・入金確認済み）', async () => {
      // 準備
      // requestのsession,userIdに正常値を入れる
      request.session = { ...session }
      request.user = { ...user[0] }
    ...
    test('準正常:csvDownloadController.createInvoiceDataForDLエラー', async () => {
      ...
      request.body = {
        ...
        buyAndSell: buyAndSell[0],
        status: status,
        minIssuedate: '9999-99-99',
        maxIssuedate: '9999-99-99'
      }
