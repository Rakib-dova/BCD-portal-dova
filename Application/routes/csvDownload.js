...
const cbPostIndex = async (req, res, next) => {
  ...
  // const fncStart = new Date().getTime()
  ...
  do {
    // 受信企業ごとの検索
    promises.push(getSentToCompanyDocuments(req, sentBy, noCompany, findDocumentQuery, sentTo[sentToIdx]))
    sentToIdx++
  } while (sentToIdx < sentTo.length)

  // 送信先別処理の終了を待機
  const retGetSentToCompanyDocuments = await Promise.all(promises)
  // 取得した請求書情報を格納
  for (const sendToResults of retGetSentToCompanyDocuments) {
    for (const sendByResults of sendToResults) {
      if (!sendByResults.documentsResult) continue
      if (!documentsResult) {
        documentsResult = {
          ...sendByResults.documentsResult
        }
      } else {
        documentsResult.itemCount += sendByResults.documentsResult.itemCount
        documentsResult.Document = documentsResult.Document.concat(sendByResults.documentsResult.Document)
      }
      // エラーの場合のみ格納（後のエラーチェックのため）
      if (sendByResults.resultForQuery instanceof Error) resultForQuery = sendByResults.resultForQuery
    }
  }

  // // アプリ効果測定用ログ出力
  // jsonLog = { tenantId: req.user.tenantId, action: `[Performance]getInvoiceList: ${new Date().getTime() - start} ms` }
  // logger.info(jsonLog)
    ...
    if (!documentsResult) {
      ...
      if (req.body.invoiceNumber || false) {
        ...
        if (documentId !== '') {
          ...
          //   action: `[Performance]createInvoiceDataForDownload(single): ${new Date().getTime() - start} ms`
          // }
          // logger.info(jsonLog)
          ...
          if (resultForDocumentId instanceof Error) {
            ...
            // // アプリ効果測定用ログ出力
            // jsonLog = {
            //   action: `[Performance]dataToJson: ${new Date().getTime() - start} ms`
            // }
            // logger.info(jsonLog)
        ...
        // jsonLog = {
        //   tenantId: req.user.tenantId,
        //   action: `[Performance]createInvoiceDataForDownload(multi): ${new Date().getTime() - start} ms`
        // }
        // logger.info(jsonLog)
  ...
  // // アプリ効果測定用ログ出力
  // jsonLog = { tenantId: req.user.tenantId, action: `[Performance]cbPostIndex": ${new Date().getTime() - fncStart} ms` }
  // logger.info(jsonLog)
  logger.info(constantsDefine.logMessage.INF001 + 'cbPostIndex')
}
...
const dataToJson = (data) => {
  ...
  const _invoice = { ...InvoiceObject }

  // InvoiceLine以外の情報設定
  const taxNameLists = setOtherInvoiceLine(data, _invoice)
...
// 受診者ごとの請求書一覧を取得
const getSentToCompanyDocuments = async (req, sentBy, noCompany, findDocumentQuery, company) => {
  let sentByIdx = 0
  if (company !== noCompany[0]) findDocumentQuery.sentTo = company
  const promises = []
  do {
    // 送信企業ごとの検索
    promises.push(getSentByCompanyDocuments(req, sentBy[sentByIdx], noCompany, findDocumentQuery, company))
    sentByIdx++
...
// 送信者ごとの請求書一覧を取得
const getSentByCompanyDocuments = async (req, sentByCompany, noCompany, findDocumentQuery, company) => {
  ...
  return {
    ...
    resultForQuery: resultForQuery
  }
}

const setOtherInvoiceLine = (data, invoice) => {
  // 必須項目チェック
  ...
  if (data.AccountingSupplierParty.Party.PostalAddress || false) {
    invoice['差出人-国/地域'] = data.AccountingSupplierParty.Party.PostalAddress.Country?.IdentificationCode.value ?? ''
