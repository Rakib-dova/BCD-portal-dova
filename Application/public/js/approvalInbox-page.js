...
function stringToMoney(stringMoney) {
  return ~~stringMoney.replaceAll(',', '')
}
...
$('#btn-insert').addEventListener('click', function () {
  const inputTarget = this.dataset.target // lineNo明細順番_lineAccountCode仕訳情報順番_input_amount
  const lineNo = inputTarget.split('_')[0].replaceAll('lineNo', '')
  const invoiceLine = $(`#invoiceLine${lineNo}`)
  const lineAccountCode = $(`#lineNo${lineNo}`)
  const invoiceLineTotal = stringToMoney(invoiceLine.querySelectorAll('.lineTotal')[0].value)
  const installmentAmounts = $(`#${lineAccountCode.id}`).querySelectorAll(`.lineNo${lineNo}_input_amount`)
  const inputInstallmentAmount = $('#inputInstallmentAmount')

  let headInstallmentAmount = invoiceLineTotal
  for (const installmentAmount of installmentAmounts) {
    if (installmentAmount !== installmentAmounts[0] && installmentAmount.id !== inputTarget) {
      headInstallmentAmount -= stringToMoney(installmentAmount.value)
    }
  }

  if (~~inputInstallmentAmount.value !== 0) {
    if (headInstallmentAmount <= stringToMoney(inputInstallmentAmount.value)) {
      $('#installmentAmountErrMsg').innerText = '計上金額の合計が小計金額を超えています。'
      return null
    }
    installmentAmounts[0].value = moneyToString(headInstallmentAmount - stringToMoney(inputInstallmentAmount.value))
    $(`#${inputTarget}`).value = moneyToString(inputInstallmentAmount.value)
