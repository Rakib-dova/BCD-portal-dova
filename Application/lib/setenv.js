exports.config = (env) => {
  const { TS_HOST, TS_API_HOST, TS_CLIENT_ID, TS_CLIENT_SECRET } = JSON.parse(env.TS_CONNECTION)
  ...
  const { DB_HOST, DB_NAME, DB_USER, DB_PASS } = JSON.parse(env.DB_CONNECTION)
  process.env.DB_HOST = DB_HOST
  process.env.DB_NAME = DB_NAME
  process.env.DB_USER = DB_USER
  process.env.DB_PASS = DB_PASS

  const { REDIS_HOST, REDIS_PORT, REDIS_PASS } = JSON.parse(env.REDIS_CONNECTION)
  ...
  const { TOKEN_ENC_PASS, TOKEN_ENC_SALT } = JSON.parse(env.TOKEN_ENC)
